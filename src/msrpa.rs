...
impl System<Time, PendulumState> for &PendulumSystem {
    fn system(&self, x: Time, y: &PendulumState, dy: &mut PendulumState) {
        *θ_mut(dy) = ω(y);
        *ω_mut(dy) = -(θ(y).cos() * anchor_vx(dy)) / self.r
            - (θ(y).sin() * anchor_vy(dy)) / self.r
            - (self.pendulum_damping * ω(y)) / (self.pendulum_mass * self.r.pow(2))
...
pub(crate) fn view(app: &App, model: &MovableSingleRigidPendulumAnal, frame: Frame) {
    ...
    draw.ellipse().radius(20.).xy(end).color(STEELBLUE);
    ...
    match event {
        ...
        MousePressed(button) => {
            if let MouseButton::Left = button {
                model.pendulum_system.mouse_stiffy = 0.1;
            }
        }
